...
SNAPSHOT_ACTIVITY = f"{APP_ID}/me.hletrd.telecampro.ui.UiSnapshotActivity"
MEDIA_DIR = "/sdcard/DCIM/TeleCamPro"
MEDIA_RELATIVE_PATH = "DCIM/TeleCamPro/"
...
class Adb:
    def __init__(
        ...
        *,
        ...
        snapshot_activity: str = SNAPSHOT_ACTIVITY,
        media_relative_path: str = MEDIA_RELATIVE_PATH,
    ):
        self.serial = serial
        ...
        self.media_dir = f"/sdcard/{media_relative_path.rstrip('/')}"
    ...
    def pid(self) -> int | None:
        out = self.shell(f"pidof {self.application_id} || true")
    ...
    def launch(self, wait_s: float = 5.0) -> int:
        ...
        self.shell(f"am start --activity-reorder-to-front -n {self.main_activity}")
        deadline = time.time() + wait_s + 10
        last_resumed: str | None = None
        while time.time() < deadline:
            p = self.pid()
            last_resumed = self.resumed_activity()
            if p and last_resumed == self.main_activity:
                time.sleep(wait_s)  # let the camera session configure
                if self.pid() == p and self.resumed_activity() == self.main_activity:
                    return p
            time.sleep(0.5)
    ...
    def locale_state(self) -> dict[str, str]:
        """Attest app override plus system fallback without changing either locale."""
        user = self.shell("am get-current-user")
        if not user.isdigit():
            raise AdbError(f"could not attest current Android user: {user!r}")
        app_locales = self.shell(
            f"cmd locale get-app-locales {shlex.quote(self.application_id)} "
            f"--user {user} 2>/dev/null || true"
        )
        system_locale = self.shell("getprop persist.sys.locale")
        override_match = re.search(r"\[([^]]*)\]", app_locales)
        override = override_match.group(1).strip() if override_match else ""
        effective = override.split(",", 1)[0].strip() or system_locale.strip()
        if re.fullmatch(r"[A-Za-z]{2,3}(?:[-_][A-Za-z0-9]+)*", effective) is None:
            raise AdbError(
                f"could not attest effective locale: app={app_locales!r} system={system_locale!r}"
            )
        return {
            "app_override_raw": app_locales,
            "user": user,
            "system": system_locale,
            "effective": effective.replace("_", "-"),
        }
    ...
    def media_store_rows(self) -> list[MediaRow]:
        ...
        where = (
            f"relative_path='{self.media_relative_path}' "
            f"AND owner_package_name='{self.application_id}'"
        )
    ...
    def tap_selector(self, selector: Selector, locale: str, retries: int = 3) -> UiNode:
        """Tap an exact semantic identity using its attested-locale label set."""
        last = None
        for _ in range(retries):
            tree = self.ui()
            node = tree.find_selector(selector, locale)
            if node is not None:
                if not node.enabled:
                    raise AdbError(f"UI selector is disabled ({selector.identity})")
                self.tap(*node.center)
                time.sleep(0.9)
                return node
            last = tree
            time.sleep(1)
        labels = ", ".join(sorted(last.all_labels())[:25]) if last else "?"
        raise AdbError(f"UI selector not found ({selector.identity}); visible: {labels}")
