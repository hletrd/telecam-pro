...
class FakeTree:
    ...
    def find_selector(self, selector, locale: str):
        return next(
            (object() for label in selector.labels_for(locale) if label in self.descriptions),
            None,
        )
    ...
    def shell(self, cmd: str, timeout: int = 60) -> str:
        ...
        if "/images/" in cmd:
            return (
                ...
                "relative_path=DCIM/TeleCamPro/, is_pending=0, width=3072, height=4096, "
                "_size=123456, owner_package_name=me.hletrd.telecampro.debug"
            )
        return (
            "Row: 0 _id=7, _display_name=VID_7.mp4, mime_type=video/mp4, "
            "relative_path=DCIM/TeleCamPro/, is_pending=1, width=2160, height=3840, "
...
class MediaStoreTest(unittest.TestCase):
    ...
    def test_media_row_parser_rejects_missing_projected_fields(self) -> None:
        output = (
            "Row: 0 _id=7, _display_name=VID_7.mp4, mime_type=video/mp4, "
            "relative_path=DCIM/TeleCamPro/, width=2160, height=3840, _size=7654321, "
    ...
    def test_read_only_foreground_helper_rejects_snapshot_activity_chrome(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            adb = GuardAdb(
                ...
                resumed_component=SNAPSHOT_ACTIVITY,
            )
