...
def assert_published_row(row: MediaRow) -> None:
    assert row.relative_path == ctx.adb.media_relative_path, f"wrong relative path: {row}"
    assert row.owner_package_name == ctx.adb.application_id, f"wrong MediaStore owner: {row}"
...
def t_launch(ctx: Context) -> None:
    ...
    tree = ctx.adb.ui()
    locale = ctx.adb.locale_state()["effective"]
    assert tree.find_selector(OPEN_SETTINGS, locale), "OSD chrome missing (no settings button)"
...
@test("localized_camera_semantics", "smoke")
def t_localized_camera_semantics(ctx: Context) -> None:
    """English/Korean smoke selectors resolve exact, non-mutating camera semantics."""
    ensure_foreground(ctx)
    locale_state = ctx.adb.locale_state()
    locale = locale_state["effective"]
    language = locale.split("-", 1)[0].casefold()
    assert language in {"en", "ko"}, f"smoke locale must be English or Korean, got {locale!r}"
    tree = ctx.adb.ui("localized_camera_semantics")
    missing = [
        selector.identity
        for selector in (OPEN_SETTINGS, OPEN_FUNCTION_MENU, PHOTO_MODE, VIDEO_MODE)
        if tree.find_selector(selector, locale) is None
    ]
    assert not missing, f"localized semantic identities missing for {locale}: {missing}"
    assert (
        tree.find_selector(TAKE_PHOTO, locale)
        or tree.find_selector(START_RECORDING, locale)
    ), f"localized shutter identity missing for {locale}"
    ctx.note(f"locale={locale}; stable camera semantics present")
...
def t_3a(ctx: Context) -> None:
    ...
    mark = ctx.adb.log_mark()
    locale = ctx.adb.locale_state()["effective"]
    mode = "VIDEO" if ctx.adb.ui().find_selector(START_RECORDING, locale) else "PHOTO"
...
def t_capture(ctx: Context) -> None:
    ...
    for row in new:
        ...
        local = ctx.adb.pull(f"{ctx.adb.media_dir}/{name}", ctx.evidence / name)
...
def _pull_and_check_rows(ctx: Context, rows: list[MediaRow]) -> list[str]:
    ...
    for row in rows:
        assert_published_row(row)
        local = ctx.adb.pull(f"{ctx.adb.media_dir}/{row.display_name}", ctx.evidence / row.display_name)
...
def t_tele_dng_parity(ctx: Context) -> None:
    ...
    try:
        ...
        for row in new:
            assert_published_row(row)
            local = ctx.adb.pull(
                f"{ctx.adb.media_dir}/{row.display_name}", ctx.evidence / row.display_name
            )
...
def t_recording_snapshot(ctx: Context) -> None:
    ...
    try:
        ...
        video_local = ctx.adb.pull(
            f"{ctx.adb.media_dir}/{published_video.display_name}",
