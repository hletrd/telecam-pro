...
data class CameraCaps(
     ...
     * [largestJpegSize] active-array filter deliberately excludes for the binned path). The
     * maximum-resolution map is read even when ULTRA_HIGH_RESOLUTION_SENSOR is absent, as permitted
     * by SENSOR_PIXEL_MODE's contract; that path still records that the still request needs maximum
     * pixel mode. PMA110 advertises neither path (probed 2026-07-22). Hi-res is session-gated
