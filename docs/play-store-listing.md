...
```
클립온 텔레컨버터를 페리스코프 망원에 물려 쓰는 오픈소스 수동 카메라.
```

### Release notes — 출시 노트 (≤500자)

v1.0.1 (`versionCode 2`) 출시 노트입니다. 영어본과 같은 규칙으로, 줄바꿈을 그대로 두고 붙여넣으세요.

```
<ko-KR>
한국어 UI를 추가했습니다.

태블릿처럼 큰 화면에서는 컨트롤을 옆 레일로 모은 가로 레이아웃을 씁니다.
...
```
TeleCam Pro는 휴대폰의 페리스코프 망원 렌즈에 클립온 아포컬 텔레컨버터를 물려 촬영하는 오픈소스 수동 카메라입니다. 이런 컨버터를 물리면 상이 180도 뒤집히는데, 그 반전을 바로잡고 망원 촬영에 필요한 수동 제어를 갖췄습니다.
...
텔레컨버터 모드
• 텔레컨버터 토글은 컨버터를 물린 렌즈를 고정하고 아포컬 180도 반전을 뷰파인더와 사진, 영상에서 함께 바로잡습니다. 렌즈 프리셋은 토글과 별개라서 화각을 바꿔도 컨버터 설정이 저절로 바뀌는 일은 없습니다.
• 물린 컨버터는 직접 고릅니다. 휴대폰을 먼저 고르면 그 기기에 물릴 수 있는 광학계만 나옵니다. Hasselblad 300 mm와 230 mm, ZEISS 200 mm와 400 mm, 범용 클립온, 직접 입력하는 배율을 지원합니다. 각 프리셋의 배율은 그 컨버터가 설계된 호스트 렌즈를 기준으로 계산하므로, 초점거리 표시와 EXIF에는 경통에 적힌 숫자가 아니라 실제 조합으로 나오는 값이 기록됩니다.
• 기기의 Camera2 OIS와 동영상 손떨림 보정을 씁니다.

렌즈
• 초점거리 프리셋은 사용 중인 휴대폰이 실제로 닿을 수 있는 것만 나오고, 뷰파인더에 현재 환산 초점거리를 함께 보여 줍니다. 크롭으로만 닿는 프리셋은 렌즈가 아니라 줌으로 표시합니다.
...
사진
• HEIF(기기가 인코딩할 수 있는 경우), JPEG, RAW(DNG)를 따로 또는 함께 저장합니다. RAW는 기기가 지원한다고 알리는 후면 렌즈에서만 쓸 수 있습니다. 촬영 당시 기기 방향에 맞춰 돌려서 저장합니다.

동영상
• HEVC 또는 H.264. 인코더가 10비트를 지원하는 기기에서는 HEVC에서 SDR과 함께 HLG, 앱이 직접 렌더링하는 S-Log3 / S-Log3.Cine / LogC3를 쓸 수 있습니다. 지원하지 않는 기기에서는 SDR만 쓸 수 있습니다. 내용과 다르게 표기된 파일은 만들지 않습니다.
• 오픈 게이트는 16:9로 자르지 않고 센서 판독 영역 전체를 기록합니다.
• 긴 초점거리에서 생기는 흔들림은 하드웨어 OIS와 EIS로 줄입니다.
• 기기 HAL이 지원하면 Sound Focus / Sound Stage 오디오 씬을 전달합니다.
• 표준과 NTSC 소수점 프레임레이트로 최대 4K UHD, 4K30 Max에서 약 99 Mbps까지. 해상도와 프레임레이트는 선택한 카메라와 인코더가 알리는 값을 그대로 따릅니다.

프레이밍과 모니터링
• 포커스 피킹, 제브라, 폴스 컬러, 그리드, 수평계, 펀치인 루페, 히스토그램, 파형, 초점 확인용 핀치 줌 확대 보기.
...
Android 13 이상이 필요합니다. OPPO Find X9 Ultra(PMA110)에서 측정했습니다. 모델 목록이 아니라 각 기기가 알리는 Camera2 지원 정보로 렌즈와 기능을 가려내므로, 다른 기기도 최대한 지원하지만 동작을 보장하지는 않습니다.
```
