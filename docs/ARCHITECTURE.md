...
| `DiagnosticTelemetry.kt` | Process-wide quota policy for recurring debug evidence. 3A, focus confidence, motion, hardware keys, zoom, and sustained-YUV diagnostics share a 180-row lifetime budget under ColorOS's measured 300-row process cap, preserving 120 rows for startup, frame gaps, recovery, and faults; noisy state is paced, stable state gets a 15 s heartbeat, and the YUV probe emits one constant-memory terminal summary. |
...
| `DngPreCaptureAllocation.kt` | Exactly-once DNG pre-capture admission and cancellable allocation owner. Camera2 dispatch begins only after the shared finite pre-native provider lane freezes one exact `PendingOutputAllocation`; lifecycle/session cancellation retires capture ownership immediately and routes any late row to bounded cleanup/recovery. |
