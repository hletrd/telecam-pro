...
| `StartupTrace.kt` | Debug-only cold-start stopwatch against a `resume`-origin clock. BUFFERS its marks and emits ONE line at `finish()`. `begin()` returns one opaque resume owner carried by its CameraController; exact-owner mark/finish/disarm plus latest-preview-request admission make stale controller/request callbacks inert. |
...
| `DngPreCaptureAllocation.kt` | Exactly-once DNG pre-capture admission and deadlined/cancellable allocation owner. Camera2 dispatch begins only after the shared finite pre-native provider lane freezes one exact `PendingOutputAllocation`; timeout, lifecycle/session cancellation, and scheduler rejection retire capture ownership immediately and route any late row to bounded cleanup/recovery. DNG lease release republishes complete still-admission truth. |
...
| `Pending allocation identity recovery` | `MediaStoreWriter` reserves finite process capacity before every image/video insert. If REGISTERED succeeds but exact provider identity is unavailable, ambiguous, or mismatched, a bounded closure retains that row and retries non-destructively until exact identity can authorize DISCARD; new output admission closes at capacity. |
