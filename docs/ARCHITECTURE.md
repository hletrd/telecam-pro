...
| `Teleconverter.kt` | The converter preset catalog (`TeleconverterProfile`: four first-party kits — Hasselblad 300 mm ≈ 4.286× and 230 mm ≈ 3.29× on 70 mm hosts, ZEISS 200 mm ≈ 2.35× and 400 mm ≈ 4.71× on 85 mm hosts — plus generic 1.5/2/3× clip-ons and CUSTOM; each magnification is written as converter-focal ÷ host-focal so it is auditable against the maker's printed figure, and a preset's product number is NEVER the reported focal — a "ZEISS 200" on this phone's 70 mm periscope is 165 mm) plus the pure helpers every focal/zoom consumer reads: `effectiveMagnification`, `normalizeMagnification`, `detectProfile`, `effectiveFocalMm`, and `teleDisplayBase(magnification)` — the function that replaced the former top-level `TELE_DISPLAY_BASE` constant when magnification became selectable. Passive glass cannot announce itself, so nothing here detects a converter: `detectProfile` matches the PHONE (`Build.MODEL`) and only pre-selects a default. This is the ONLY model-string branch in the codebase and it may never gate a capability, route, or request — every other hardware fact is resolved by enumerating Camera2 capabilities. |
