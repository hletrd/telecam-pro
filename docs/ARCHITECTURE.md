...
| `Teleconverter.kt` | The optics catalog as a PAIR: `PhoneModel` (host tele focal + `Build.MODEL` codes) and the `TeleconverterProfile`s that clamp onto each — Hasselblad 300 mm ≈ 4.286× and 230 mm ≈ 3.29× on 70 mm OPPO hosts, ZEISS 200 mm ≈ 2.35× and 400 mm ≈ 4.71× on 85 mm vivo hosts — plus generic 1.5/2/3× clip-ons and CUSTOM, which fit anything. A kit's magnification is converter-focal ÷ ITS OWN host focal (auditable against the maker's printed figure, and unchanged by which phone is selected — moving glass does not regrind it), and a preset's product number is NEVER the reported focal: a "ZEISS 200 mm" is 2.35× glass, which on this phone's 70 mm periscope is 165 mm. Plus the pure helpers every focal/zoom consumer reads: `effectiveMagnification`, `normalizeMagnification`, `detectPhone`, `defaultConverterFor`, `reconcileConverter`, `effectiveFocalMm`, and `teleDisplayBase(magnification)` — the function that replaced the former top-level `TELE_DISPLAY_BASE` constant when magnification became selectable. Passive glass cannot announce itself, so nothing here detects a converter: `detectPhone` matches the PHONE (`Build.MODEL`) and only pre-selects the phone dropdown. This is the ONLY model-string branch in the codebase and it may never gate a capability, route, or request — every other hardware fact is resolved by enumerating Camera2 capabilities. |
...
| `DeviceProfile.kt` | ONE authority for the front pre-mirrored-stream device fact (`frontStreamPreMirrored`), alongside the other measured HAL quirks: derives the preview/encoder/analysis draw mirror roles and the tap display axis so a re-diagnosis is a one-flag edit, never three drifting literals. Absorbed the former `FrontMirrorConvention.kt` when the multi-device profile seam landed. |
...
| FRONT (any mode) | **Front camera** (`pickFront`, expected id 1 — never hardcoded) | Lens-local zoom; one camera in both modes (mode flips change stream size only). A first-class optics door (`setFrontCamera`): entering forces TC off in the same transaction, lens presets/TC refuse while FRONT (`backOpticsDoorRefusal`), MR recall/settings restore exit FRONT (recalled packets are rear-route optics), and facing is never persisted (fresh launch = BACK). Mirror truth: the front HAL PRE-mirrors its stream, so the PREVIEW draws it as-is (the stream IS the selfie-mirror view) and the ENCODER/ANALYSIS draws un-mirror — saved files keep the true scene. Roles derive from `camera/DeviceProfile.kt`; the fact is pushed as route state by `gl.setFrontStreamPreMirrored` (replayed by `applyStabilization` like the rotation pair). |
