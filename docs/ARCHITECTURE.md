...
| `StandbyAudioController.kt` | Owns the armed-video standby meter lifecycle, bounded AudioRecord recreation, and exact `StandbyMeterOwnership` handoff to REC. Native create/start/stop/release use one process-gate publication token: the concrete input is bound before acquisition returns current, and quarantine strongly retains it before abandonment or waiter release. All engine dependencies are live lambdas so a retired meter generation cannot reclaim a newer intent. |
...
| `DiagnosticTelemetry.kt` | Pure quota policy for debug 3A and sustained-YUV evidence: sensor jitter is bucketed, tuple changes are paced, stable state gets a 15 s heartbeat, and the YUV probe accumulates one constant-memory terminal cadence summary instead of logging once per second. |
