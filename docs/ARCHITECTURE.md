...
| `DiagnosticTelemetry.kt` | Process-wide quota policy and single admission door for repeatable debug evidence. 3A, focus confidence, motion, hardware keys, zoom, standby/audio shape, capture-family, ShutterLag/ZSL, and sustained-YUV diagnostics share a 180-row lifetime budget under ColorOS's measured 300-row cap, preserving 120 rows for startup, frame gaps, recovery, and faults. |
...
| `Pending allocation identity recovery` | `MediaStoreWriter` reserves finite process capacity before every image/video insert. If REGISTERED succeeds but exact identity cannot freeze, a bounded closure retains typed exact/absent/uncertain truth and one daemon scheduler retries with 250 ms→30 s exponential backoff, one scheduled/in-flight attempt per claim. Exact identity authorizes DISCARD; stable absence clears only REGISTERED metadata; uncertain states remain fail-closed. New output admission closes at capacity, and reserve/retry/terminal edges publish through the process signal. |
