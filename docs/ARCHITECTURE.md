...
the user asks. Important states belong in the OSD, Fn, or menu rows. This paragraph plus
[`CLAUDE.md`](../CLAUDE.md) is the committed clean-clone authority; the optional private
[`UX_POLICY.md`](UX_POLICY.md) adds maintainer examples when present.
...
|---|---|
| `ProcessAdmissionSignal.kt` | Process-lifetime, change-gated availability signal with exact closeable subscriptions. DNG and MediaStore capacity owners publish every acquire/release/reserve/retry terminal through it; Engine replacement subscribes to current truth, while close drains an in-flight old-Engine callback before its UI graph detaches. CameraEngine feeds both process callbacks and local retained-family edges through one serialized conjunction owner, so an older callback cannot arrive after newer truth and no direct local publication can bypass its delivered-state cache. |
| **camera/** | |
...
| `CameraTeardownTerminal.kt` | Android-free exactly-once Camera2 teardown classification. Strict handler completion authorizes replacement; timeout/failure first closes process native admission, retains the uncertain owner, and makes late completion inert. |
...
| `CameraStatus.kt` | Typed user-status identity, arguments, urgency/live-region policy, lifecycle, and duration. Camera bring-up/reconfiguration/recovery/retry conditions are untimed `PROGRESS`; exact Ready, terminal, pause, or replacement events retire them. |
...
| `ProcessedSnapshotBudget.kt` | Android-free, process-wide admission budget for every retained processed still snapshot: one running save plus one waiting full-resolution snapshot across SINGLE, BURST, AEB, timelapse, and Engine replacement. Sequence drives additionally retain their per-Engine completion chaining for order. |
| `StillPublicationDispatcher.kt` | Per-Engine closeable facade over one process-lifetime finite owner for every completed DNG publication: two daemon workers plus two queued tails. RAW-only shots transfer directly; mixed-output shots queue only a lightweight transfer behind processed-sibling completion before entering the same owner. `transferCompletedDngFromCameraCallback` is the tested production composition of actual formats, processed-queue truth, ordered Engine enqueue, process dispatch, and retained recovery. Accepted work retains exact family/callback identity across Engine replacement; overflow/shutdown/transfer rejection never publishes inline or deletes complete bytes, and launch recovery owns the private row. |
| `FamilyDeletionMarkerDispatcher.kt` | Pre-marker family-delete admission owner: every Engine facade shares one process-lifetime daemon worker plus 31 family backlog slots. Capacity is reserved before the Engine publishes its in-memory tombstone, so overflow/shutdown restores the frozen review without running marker/provider work inline or claiming nonexistent recovery authority. Accepted completions use numeric tokens in a detach-and-drain registry, preventing queued work from retaining a stale ViewModel callback. |
...
| `StandbyAudioController.kt` | Owns the armed-video standby meter lifecycle, bounded AudioRecord recreation, and exact `StandbyMeterOwnership` handoff to REC. Native create/start/stop/release use one process-gate publication token: every constructed input (including validation failures) is bound before acquisition returns current, and any timeout, exception, rejected return, or unproven release quarantines and strongly retains it before abandonment or waiter release. All engine dependencies are live lambdas so a retired meter generation cannot reclaim a newer intent. |
...
| `StartupTrace.kt` | Debug-only cold-start stopwatch against a `resume`-origin clock. BUFFERS its marks and emits ONE line at `finish()`. `EngineStartupTraceOwnership` binds a fresh opaque owner to one exact Engine resume/open transaction, passes it explicitly into the installed controller, and revokes it on pause/release/supersession/replacement; no wiring path samples global current state. Exact-owner mark/finish/disarm plus latest-preview-request admission make stale controller/request callbacks inert. |
| `DiagnosticTelemetry.kt` | Executable process-wide quota policy for the complete production log surface. Repeatable information shares a 180-row lifetime owner; every warning/error crosses a separate 120-row owner, so the runtime maximum is the measured ColorOS 300-row cap rather than arithmetic in a test. `PreviewFrameTiming` separates cached render cadence from real SurfaceTexture frames, and `FrameGapAccumulator` emits constant-memory 15-second/terminal count, maximum, and duration-bucket summaries through the recurring owner. |
...
| `DngPreCaptureAllocation.kt` | Exactly-once DNG pre-capture admission and deadlined/cancellable allocation owner. Camera2 dispatch begins only after the shared finite pre-native provider lane freezes one exact `PendingOutputAllocation`; timeout, lifecycle/session cancellation, and scheduler rejection retire capture ownership immediately and route any late row to bounded cleanup/recovery. Every DNG acquire/release publishes through the exact closeable process admission signal, so replacement Engines observe the current shutter truth. |
| `RecordingPreNativeAllocation.kt` | Process-wide finite pre-native MediaProvider lane shared by pending-video and DNG allocation: two daemon workers plus four queued attempts, per-attempt first-wins retirement, and cancellation of queued work. Stop/pause/release/timeout free caller admission immediately; a late row is cleanup/recovery-owned and cannot enter recorder/Camera2 setup. |
...
| `LatestCaptureReducer.kt` | Android-free reducer for package-owned and owner-null Images/Video rows. Selects the newest capture first, then a displayable sibling inside only that capture. Current-package ownership is verified; a null owner plus the public directory/name/collection/extension/MIME contract is labeled `LEGACY_FORMAT_UNVERIFIED`, never app-authored. Unverified/legacy media keeps file-only deletion. |
| `MediaStoreWriter.kt` | Scoped-storage wrapper with durable per-URI `REGISTERED`/`COMPLETE`/`DISCARD` ownership and a bounded family-delete journal. Family deletion, live still publication, and pre-Camera2 process producer leases share one process-wide reference-counted exact-family authority. A lease survives Engine replacement until every processed/DNG lane is terminal, so retirement cannot erase a current-process tombstone before a future sibling creates its row or publication claim. Publication/producer claims install atomically with registry admission before waiting. Retirement seals a claim-free exact-family decision before slow marker removal; a registrant that arrives after that seal waits without the global registry and becomes a post-retirement admission. Publication then holds only that family's owner through the authoritative marker read, provider `IS_PENDING=0`, and exactly-once saved callback. Delete that commits first selects durable DISCARD without publication; publication that wins first completes before Delete and is found by the exact-family untracked-row sweep. Retirement requires zero producer leases and zero queued publication claims around its absence proof. Thread-safe read-only marker checks do not take the global capacity/RMW lock; provider calls/retry sleeps also never hold short global registry/marker-capacity monitors, so unrelated families continue independently. Rejected outputs always attempt typed DISCARD before deletion; double-failures stay in one finite process retry owner and close new output admission at capacity. COMPLETE exhaustion or an unreadable family marker leaves structurally complete bytes private. Launch recovery adopts COMPLETE/valid rows, deletes only proven-invalid or durably rejected/deleted rows, and retains indeterminate/error rows through a bounded preflight, independent Images/Video pages, and a separately cursor-paged durable-DISCARD stage. An unreadable exact-DISCARD authority is a typed retry failure before probing/adoption/publication, never evidence of marker absence. |
| `SharedPreferencesDurableEdit.java` | Minimal Java bridge that returns the Boolean result of synchronous `SharedPreferences.Editor.commit()`. `MediaStoreWriter` uses it where REGISTERED/COMPLETE/DISCARD durability must fail closed; Kotlin KTX `edit(commit = true)` returns `Unit` and cannot preserve that result. |
| `PendingDiscardJournal.kt` | SQLite-backed exact-row DISCARD owner. Schema v2 additively preserves v1/SharedPreferences URI-only entries as explicit legacy records, while every new marker first captures a mounted specific MediaStore volume, its opaque provider version, `_ID`/`GENERATION_ADDED`, display name, relative path, MIME, owner, reconstructible capture family, and date. Replay rereads that complete identity and rechecks provider version; only an exact match reaches an atomic identity-constrained provider delete. Stable absence may retire metadata only under the same volume/version. Version changes, removed volumes, unreadable/ambiguous or incomplete identities, URI reassignment, and legacy rows retain their marker fail-closed. Lookup remains tri-state (`PRESENT` / `ABSENT` / `UNAVAILABLE`) so database failure cannot resurrect delete-owned media. A process-wide reference-counted exact-URI authority serializes publication with marker creation/removal/replay for only that URI; the separate SQLite monitor never spans MediaProvider Binder I/O, retry sleeps, legacy SharedPreferences loading, or bounded per-page preference cleanup. The primary-key index still provides `uri > cursor ORDER BY uri LIMIT 65` paging, and every page releases database ownership before best-effort preference cleanup; the one-time legacy import remains bounded and idempotent. REGISTERED/COMPLETE remain preference-backed. |
| `Pending allocation identity recovery` | `MediaStoreWriter` reserves finite process capacity before every image/video insert. A failed REGISTERED commit is classified separately from insert failure: the reservation is released only after provider deletion/absence and preference cleanup are authoritative; otherwise the URI/family transfers to the same bounded identity owner. After REGISTERED, an identity that cannot freeze likewise retains typed exact/absent/uncertain truth. One daemon scheduler retries with 250 ms→30 s exponential backoff and one scheduled/in-flight attempt per claim. Exact identity authorizes DISCARD; stable absence clears only REGISTERED metadata; uncertain states remain fail-closed. New output admission closes at capacity, and reserve/retry/terminal edges publish through the process signal. |
...
| `OwnerlessMediaDeleteOperation.kt` | Exact generation identity and first-wins deadline terminal shared by ownerless MediaStore request construction and post-consent presence reconciliation. A timed-out Binder task drops its Activity/ViewModel callback, restores truthful unknown presence, and cannot publish a late request or result. |
...
| `CameraInputBlockOwner.kt` | Identity-owned reducer for full-screen Compose, review, permission, policy, and system-delete input blocks; one owner can never release another across recreation or overlapping surfaces. |
...
| `LatestHeavyWorkLane.kt` | Identity-owned review-work boundaries. Every still request freezes one private no-follow compressed source for displayed EXIF, orientation, bounds, and pixels; owner-unverified media keeps a 64 MiB ceiling, while app-owned hi-res input is bounded by a 512 MiB hard cap, one quarter of usable cache, and a two-source process budget. Full and partial spool deletion release byte accounting only after no-follow absence; failed deletion remains in a finite exact-file cleanup owner and closes new admission at capacity. Crash-stale spools are reclaimed by a bounded schema-only scan. Production uses `ProgressiveLatestWorkLane`: two consumers per logical lane, one conflated pending request, and one shared four-daemon process pool. Its atomic QUEUED→STARTED→PRODUCED→TERMINAL owner distinguishes a retryable active-call timeout from a queued request that could not acquire poisoned capacity, while a completion racing the 5 s deadline is either published or disposed exactly once. The older serial `LatestHeavyWorkLane` remains a focused ownership seam rather than a production acquisition owner. |
...
| `MainActivity.kt` | Entry point. Requests CAMERA/RECORD_AUDIO at runtime, and the READ_MEDIA trio contextually (ColorOS blocks pm grant). CAMERA request history distinguishes fresh/cancelled prompts from fixed denial before offering Settings. Activity dispatch rejects both full and partial Android obscuration flags before Compose, while unflagged pointer input is delegated unchanged. Hosts the Compose root and ViewModel. Lifecycle: `onStart` calls the ViewModel's `onStart`, which resumes the engine; `onStop` calls the ViewModel's `onStop`, which pauses it. |
| `DebugCameraControlCommand.kt` | Process-local mailbox carrying shell measurement commands from the debug-only DUMP-protected component to MainActivity. The ordinary exported launcher never parses command extras; release has no exported mailbox producer. |
...
| `HardwareInputPolicy.kt` | Pure camera-key EDGE ownership (`cameraKeyDecision` / `updateAggregateCameraKeyOwnership`) pairs key-down claims with their key-ups across the aliased camera keycodes, while `hardwareActionAdmitted` keeps an unavailable SHUTTER inert without leaking the owned key into volume/system behavior. The OEM keycode families and their dispatch to configurable actions live in `MainActivity.kt`; the `HardwareKeyAction` enum lives in `camera/CameraState.kt`. |
...
3. Cannot pixel-rotate Bayer CFA; EXIF tag is auto-applied by RAW renderers. Every completed DNG
   publication crosses to the process-wide finite still-publication owner (two active + two queued).
   RAW-only shots transfer directly; mixed output transfers only after processed completion, so
   sibling and sequence ordering remains exact. Publication still requires the marker to be durable. Marker exhaustion, capacity
...
| **review media-delete dispatcher** (process-wide one worker + eight FIFO backlog slots) | `ProcessViewModelMediaDeleteOwner`; each CameraViewModel owns one closeable admission facade | Serial exact-family sweeps, known-output deletes, rejected-late-sibling disposal, ownerless delete-request construction, and post-consent presence checks. Accepted old-ViewModel work keeps its frozen identity; replacement shares the same finite capacity. Ownerless request/query work additionally has an independent 5 s first-wins UI deadline, so a queued or active Binder wedge restores the frozen handle, clears modal input ownership, drops its callback graph, and makes a late result inert. Refusal never blocks or calls MediaProvider inline: a whole-family marker stays durable for recovery, file-only review ownership is restored for retry, and terminal UI copy points to Gallery. |
...
| **ioExecutor** (single-thread) | CameraEngine / StillCapturePipeline | Deferred processed-still decoding, crop/rotation, shared HEIF/JPEG EXIF composition, encoding, and processed publication. For mixed DNG output it queues only the lightweight transfer to the process still-publication owner after the processed sibling terminal. |
| **family-deletion marker dispatcher** (process-wide one worker + 31 backlog slots) | `ProcessFamilyDeletionMarkerOwner`; each CameraEngine owns one closeable admission facade | Durable capture-family marker commit and its exact completion edge before provider deletion begins. Admission reserves the family before the Engine tombstone, overflow/shutdown fails promptly without inline I/O, and accepted work survives facade replacement within a fixed process ceiling. |
| **still-publication dispatcher** (process-wide two workers + two backlog slots) | `ProcessStillPublicationOwner`; CameraEngine owns one closeable admission facade | Every completed DNG publication after synchronous byte write and COMPLETE-marker attempt. RAW-only transfers directly; mixed output transfers after processed completion. Admission is non-blocking and finite across Engine generations. Accepted tails retain exact capture-family callback identity; their final marker check, provider publication, and saved callback run under the process-wide exact-family authority shared with Delete. Overflow/facade/transfer refusal reports delayed recovery, terminally releases the live family continuation, leaves complete bytes private, and never calls MediaProvider inline. |
| **rejected-output cleanup dispatcher** (process-wide two workers + eight backlog slots) | `MediaStoreWriter`; RAW capture reserves one slot before Camera2 dispatch | Failed DNG identity capture, DISCARD SQLite work, provider deletion, retries, and result callbacks. The live-RAW camera callback performs only DNG byte work and the required COMPLETE-marker attempt; failed-row cleanup is always handed to this finite process owner. Overflow/shutdown leaves REGISTERED recovery truth and never calls the provider inline. |
| **media-recovery executor** (one process-wide daemon + one watchdog daemon) | `ProcessLaunchMediaRecovery`; Engines own cancellable subscribers | Single-flight launch-only recovery with bounded retry/backoff: one family/rejected-output preflight, independent 64-row monotonic Images and Video pages, then indexed SQLite DISCARD pages that read at most 65 rows. Legacy import commits once before paging; failed preference cleanup never repeats the full import. DISCARD-authority or identity unavailability, provider-version change, URI reassignment, ambiguity, and legacy URI-only records all retain the row and retry as DELETE without touching the provider row. An exhausted returned-failure page retains its markers and advances. A separate 120 s watchdog terminally poisons a non-returning provider worker without interrupting or replacing it, makes late completion inert, and delivers the same typed failure to every current/later subscriber until process restart; that terminal still gates the safe latest-family query. |
| **retained-still discard dispatcher** (process-wide two workers + eight backlog slots + conflated immediate/delayed retirement rescans) | `ProcessRetainedStillDiscardOwner`; CameraEngine owns one closeable admission facade | Provider retirement/discard for completed late stills and every deleted-family retirement recheck. Retirement never enters the Engine's unbounded still-encoding queue, so independently completed RAW tails cannot accumulate behind a wedged encode. Accepted old-Engine work finishes under its exact deletion owner. A process producer registered before release may submit its final retirement recheck directly to this same finite lane after the old facade closes. Ordinary discard overflow remains durably launch-recovery-owned. Queue overflow performs no inline provider work: one constant-memory rescan re-arms after capacity frees. An accepted result distinguishes authoritative row presence from retryable marker/query/removal or publication contention; only the latter arms one process-owned exponentially backed-off rescan (250 ms to 30 s), so a genuinely live row never drives a timer loop. Every scan is bounded by the journal's 64 current-process families. The registry uses that same distinct-family bound and a separate four-listener per-family fan-out, drains exact owners on terminal retirement, and unregisters a released Engine so delayed retries cannot retain its callback graph. |
...
| **pre-native media allocation** (process-wide two workers + four backlog slots) | CameraEngine / `ProcessPreNativeMediaAllocator` | Pending video-row allocation before recorder setup and pending DNG row/identity allocation before Camera2 dispatch. Stop, pause, release, timeout, or session supersession retires caller admission without interrupting an uncancellable Binder call; late exact allocations can only enter bounded cleanup/recovery. |
...
| **timelapseScheduler** (scheduled) | CameraEngine | Interval-driven timelapse capture trigger; each retry/save-completion boundary rereads the live interval, so an accepted edit applies to the next scheduled frame rather than only the next run. |
...
3. MediaStore: create `REGISTERED` pending row → write/close → attempt to mark `COMPLETE`; return
   a `PendingDngPublication` carrying marker durability from the camera callback. Every completed DNG
   calls `publishDng` through the process-wide two-worker/two-backlog still-publication owner;
   mixed-output transfer waits behind the processed sibling terminal. If marking,
...
authorship, so the reducer/UI label it `LEGACY_FORMAT_UNVERIFIED` and keep file-only deletion. A
failure in one collection does not discard valid rows from the other. The Android-free reducer chooses the newest capture
...
provider deletion. Every still also registers a process producer lease before Camera2 dispatch and
releases it only after all processed/DNG lanes are terminal; restored deletion therefore cannot
retire the current-process marker while an old Engine still owns a not-yet-created sibling. Still
publication and producer claims install atomically with the same exact-family registry admission
before waiting. A publication keeps that authority through its marker decision, provider visibility,
...
- `docs/BACKLOG.md` — optional private maintainer release/deferred-work history when present. In a
  clean clone, this architecture plus `CLAUDE.md` and `docs/FIELD_CHECKS.md` are the committed
  authority and field-results ledger.
- `docs/superpowers/specs/2026-07-01-find-x9-ultra-camera-design.md` — optional private historical
  snapshot when present; superseded by this current architecture/code wherever it differs.
