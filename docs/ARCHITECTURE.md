...
| `MediaStoreWriter.kt` | Scoped-storage wrapper with durable per-URI `REGISTERED`/`COMPLETE`/`DISCARD` ownership and a bounded family-delete journal. Family deletion and live still publication share one process-wide reference-counted exact-family authority: publication registers before waiting, then holds only that family's owner through the authoritative marker read, provider `IS_PENDING=0`, and exactly-once saved callback. Delete that commits first selects durable DISCARD without publication; publication that wins first completes before Delete and is found by the exact-family untracked-row sweep. Retirement retains its marker when a queued publication claim arrived during the absence query. Provider calls/retry sleeps never hold the short global registry/marker-capacity monitors, so unrelated families continue independently; a same-family re-mark still waits behind old retirement. Rejected outputs always attempt typed DISCARD before deletion; double-failures stay in one finite process retry owner and close new output admission at capacity. COMPLETE exhaustion or an unreadable family marker leaves structurally complete bytes private. Launch recovery adopts COMPLETE/valid rows, deletes only proven-invalid or durably rejected/deleted rows, and retains indeterminate/error rows through a bounded preflight, independent Images/Video pages, and a separately cursor-paged durable-DISCARD stage. An unreadable exact-DISCARD authority is a typed retry failure before probing/adoption/publication, never evidence of marker absence. |
...
provider deletion. Still publication registers with the same exact-family authority before its
