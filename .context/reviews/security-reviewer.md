...
- **Why it's a problem:** The v1 release ships without R8 shrinking/obfuscation. This is intentional per the comment, but it enlarges the static-analysis surface and ships all diagnostic code (e.g., Finding 1) and full symbol names. Minification is not a control by itself, but its absence removes a defense-in-depth layer and leaves dead diagnostic paths in the shipped APK.
